...
# Enter commands in R (or R studio, if installed)y
...
results_folder = '~/Documents/Resolve/Giotto_loader_Resolve/'

# Upload resolve object
resolve.obj <- readRDS("~/Documents/Resolve/Giotto_loader_Resolve/Cellpose.dragonfly.W6A1/cellpose.dragonfly.W6A1.rds")
...
# Run the clustering using the Leiden algorithm from the specified module
resolve.obj <- FindClusters(resolve.obj, verbose = FALSE, resolution = 0.6, algorithm = 4)

#resolve.obj <- FindClusters(resolve.obj, verbose = FALSE, resolution = 0.6, algorithm = 'leiden')
